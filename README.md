...
1. **Git guard** — prevents agents from committing/pushing to protected repos. Uses an env var (`AGENT_FLAG_*`) set by the agent's extension, checked by global git hooks.
